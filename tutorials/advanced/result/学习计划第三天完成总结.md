...
- **代码阅读**：分析 [MemTable.java](../../../src/main/java/com/brianxiadong/lsmtree/MemTable.java) 的源码，理解其核心实现，包括数据插入、删除、查询以及刷盘（Flush）触发机制。
...
通过阅读 [tutorials/03-memtable-skiplist.md](../../../tutorials/03-memtable-skiplist.md) 和 [docs/04-source-code-analysis.md](../../../docs/04-source-code-analysis.md)，我深入比较了不同数据结构在 MemTable 场景下的优劣，最终理解了为什么跳表是理想的选择。
...
## 3. [MemTable.java](../../../src/main/java/com/brianxiadong/lsmtree/MemTable.java) 源码分析

通过对 [src/main/java/com/brianxiadong/lsmtree/MemTable.java](../../../src/main/java/com/brianxiadong/lsmtree/MemTable.java) 的源码分析，我掌握了其内部实现细节。
...
- **触发机制**：实现非常简洁，一旦 `currentSize` 达到或超过 `maxSize` 阈值，该方法就返回 `true`，外部调用者（[LSMTree.java](../../../src/main/java/com/brianxiadong/lsmtree/LSMTree.java)）会据此启动刷盘流程。
...
我创建了 [MemTableTest.java](../../../src/test/java/com/brianxiadong/lsmtree/MemTableTest.java) 文件，包含针对 MemTable 的全面测试用例，涵盖以下场景：
...
- **测试用例设计挑战**：在创建 [MemTableTest.java](../../../src/test/java/com/brianxiadong/lsmtree/MemTableTest.java) 时，最初对 `size()` 方法的行为理解有误，导致测试失败。通过仔细分析源码，发现 `size()` 只统计新插入的键，更新现有键不会改变计数。修正测试用例后，所有测试顺利通过。
...
- **提升了源码分析能力**：通过对 [MemTable.java](../../../src/main/java/com/brianxiadong/lsmtree/MemTable.java) 的逐行分析，学习到了如何在关键性能路径上做精巧的设计（如 `volatile` 的使用）。
- **掌握了 MemTable 的测试方法**：通过创建 [MemTableTest.java](../../../src/test/java/com/brianxiadong/lsmtree/MemTableTest.java)，学习了如何设计全面的测试用例，包括功能测试、性能测试和并发测试，确保代码的正确性和稳定性。
