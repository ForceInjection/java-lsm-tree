...
./test-suite.sh memory-opt    # 内存优化专项测试（百万级数据）
...
#### 3.1.4 内存优化专项测试 (memory-opt)

- **百万级数据测试**: 专门针对100万条记录的内存优化测试
- **性能对比分析**: 标准实现 vs 优化实现的详细性能对比
- **GC压力评估**: 测量垃圾回收次数和时间的减少情况
- **对象池效率**: 验证对象池使用率和命中率统计
- **内存使用优化**: 分析堆内存和非堆内存的增长情况
